...
export class BancoCentralScrapper extends NoticiaScrapper {
    public buscarNoticias(): Promise<Noticia[]> {
    ...
    public async lerNoticiasDoAno(ano: number): Promise<Noticia[]> {
        ...
        for (let i = 0; i < cardElements.length; i++) {
            ...
            for (let j = 0; j < noticiaElements.length; j++) {
                ...
                if (data === undefined) {
                    console.warn('Noticia sem data, seguindo para a proxima');
                    continue;
                }
                ...
                noticias.push({
                    'data': `${ano}-${this.mesParaNumero(mes)}-${data.substring(data.length - 2).trim()}`,
                    'titulo': titulo ?? "",
                    'descricao': descricao ?? "",
    ...
    public async lerNoticiasDosAnos(anos?: number[]): Promise<Noticia[]> {
        ...
        for (const ano of anos) {
            for (const noticia of await this.lerNoticiasDoAno(ano)) {
